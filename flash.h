...
struct wp_chip_config;
struct wp_range;

/* A function that takes WP-related chip config (BP, SEC, TB, ...) and
 * determines what protection range is selected by it. */
typedef void (*wp_range_decode_fn_t)(const struct flashrom_wp_chip_config *cfg, uint32_t chip_len, struct flashrom_wp_range *range);
